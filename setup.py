...
if BUILD_CUDA_EXT:
    ...
    if os.name == "nt":
        # handle unresolved external symbol cublasHgemm
        extra_link_args += ["cublas.lib"]
        # switch MSVC to a standards conforming preprocessor and use win32 headers
        extra_compile_args["nvcc"] += [
            "-Xcompiler=/Zc:preprocessor",
            "-D_WIN32=1",
            "-DUSE_CUDA=1",
            # fixes torch issue with "small" macro on some versions
